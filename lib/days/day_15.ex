defmodule Advent2020.Days.Day15 do
  ...
  def part_one do
    play_memory_game(@input)
  end

  def play_memory_game(starting_numbers) do
    memory = starting_numbers
      |> Stream.with_index(1)
      |> Enum.into(%{})
    turn = map_size(memory) + 1
    previously_spoken = List.last(starting_numbers)
    play_turns(memory, turn, previously_spoken)
  end

  defp play_turns(_memory, 2021, previously_spoken), do: previously_spoken
  defp play_turns(memory, turn, previously_spoken) do
    ...
    play_turns(new_memory, turn + 1, new_number)
  end
end
