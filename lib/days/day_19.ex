defmodule Advent2020.Days.Day19 do
  ...
  def part_two do
    # instead of a list create a stream, zip with the inputs?
  ...
  def find_matches_pt2(messages, rules) do
    ...
    # 8: 42 | 42 8
    # 11: 42 31 | 42 11 31
    # take from 8 until it doesn't match?
    # enumerate all pairs up until the longest message?
    ...
    |> Enum.filter(fn message ->
      ...
      # IO.puts "thirty_one: #{thirty_one_matches_count}"
      # IO.puts "forty_two: #{forty_two_matches_count}"
      # IO.puts "matched full: #{matched_full_string}"
      # IO.puts "good ratio: #{ratio_is_good}"

      thirty_one_matches_count > 0
      and ratio_is_good
      and matched_full_string
    end)
  end
